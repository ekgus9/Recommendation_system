...
sheet = wb.active
sheet['A1'] = '작품' ; sheet['B1'] = '유사작품코드' ; sheet['C1'] = '유사작품제목'

for i in range(len(text)):
    result_index = recommendations(text.iloc[i][0])
    result = [str(x + y) for x, y in zip(result_index,[1]*len(result_index))]
    sheet[f'A{i+2}'] = i+1
    sheet[f'B{i+2}'] = ','.join(result)
    
    sim_list = []
    for j in range(len(result_index)):
        sim_list.append(text.iloc[result_index[j]][0])
    sheet[f'C{i+2}'] = '*'.join(sim_list)
  
wb.save('netflix_sim.xlsx')